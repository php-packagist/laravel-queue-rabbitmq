...
namespace PhpPackagist\LaravelQueueRabbitMQ\Queue;
...
use PhpPackagist\LaravelQueueRabbitMQ\Queue\Jobs\RabbitMQJob;
...
class RabbitMQQueue extends Queue implements QueueContract
{
    ...
    public function pushRaw($payload, $queue = null, array $options = [])
    {
        ...
        $this->retry(
            fn () => $this->channel->basic_publish($message, $exchange, $destination, true, false), 3
        );
    ...
    public function pop($queue = null)
    {
        try {
            ...
            throw new AMQPRuntimeException('Lost connection: '.$exception->getMessage(), $exception->getCode(), $exception);
        }
     ...
     */
    protected function retry(callable $callback, $times = 3)
    {
        beginning:
        --$times;
        ...
        try {
            $callback();
        } catch (AMQPConnectionClosedException|AMQPIOException|RuntimeException|ErrorException $e) {
            if ($times < 1) {
                throw $e;
            }
